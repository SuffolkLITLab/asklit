...
def main():
    ...
    uploaded_files = st.file_uploader("Choose files", accept_multiple_files=True, type=['pdf', 'docx', 'txt', 'md', 'html'])
                ...
                with st.spinner(f"Processing {uploaded_file.name}..."):
                    ...
                    try:
                        ...
                        cursor.execute(
                            ...
                            (file_id, uploaded_file.name, file_path, ext, uploaded_file.size, content_hash, 'indexing')
                        )
                        ...
                        conn = get_connection()
                        cursor = conn.cursor()
                        cursor.execute("UPDATE documents SET status = 'indexed' WHERE id = ?", (file_id,))
                        conn.commit()
                        conn.close()
    ...
    conn = get_connection()
    df = pd.read_sql_query("SELECT id, filename, file_path, file_type, status, created_at FROM documents", conn)
    conn.close()
    
    if not df.empty:
        if st.button("🔄 Reindex All Documents", help="Rebuilds the vector index from the files currently in the repository."):
                ...
                try:
                    ...
                    for _, row in df.iterrows():
                        ...
                        for chunk in chunks:
                            cursor.execute(
                                "INSERT INTO document_chunks (document_id, chunk_index, content, page_number) VALUES (?, ?, ?, ?)",
                                (row['id'], chunk['chunk_index'], chunk['content'], chunk['page_number'])
                            )
        ...
        for i, row in df.iterrows():
            ...
            col1.write(f"**{row['filename']}** ({row['file_type']}) - {row['created_at']}")
            if col2.button("Delete", key=row['id']):
                with st.spinner("Deleting..."):
                    ...
                    cursor.execute("SELECT file_path FROM documents WHERE id = ?", (row['id'],))
