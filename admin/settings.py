...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["General Settings", "Model & RAG", "Prompt Engineering"])

    with tab1:
        st.header("General Settings")
        
        site_title = st.text_input("Site Title", value=get_setting("app.title", "AskLit"))
        welcome_msg = st.text_area("Welcome Message", value=get_setting("app.welcome_message", "Welcome! How can I help you today?"))
        ...
        logging_enabled = st.checkbox("Enable Usage Logging", value=(get_setting("logging.enabled", "true") == "true"))
    ...
    with tab2:
        ...
        temp = st.slider("Temperature", 0.0, 1.0, float(get_setting("model.temperature", 1.0)), 0.1)
        
        disable_temp = st.checkbox("Force Disable Temperature", 
        ...
        local_embed_model = st.text_input("Local Embedding Model", 
                                        value=get_setting("model.local_embedding_model", "all-MiniLM-L6-v2"),
                                        help="This model runs locally on your CPU/GPU.")
        
        remote_embed_model = st.text_input("Remote Embedding Model", 
        ...
        max_tokens = st.number_input("Max Output Tokens", value=int(get_setting("model.max_tokens", 1000)))
        ...
        top_k = st.number_input("Retrieval Top-K", value=int(get_setting("retrieval.top_k", 5)))
        
        show_citations = st.checkbox("Show Citations", value=(get_setting("retrieval.show_citations", "true") == "true"))
        ...
        if st.button("Save Model Settings"):
            ...
            set_setting("retrieval.show_citations", "true" if show_citations else "false")
    ...
    with tab3:
        ...
        new_prompt = st.text_area("Active System Prompt", value=current_prompt, height=400)
